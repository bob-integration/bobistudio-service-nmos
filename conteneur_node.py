...
# ══════════════════════════════════════════════════════════════════════════════════════════════
# Pas de Receiver d'événements ICI — et c'est une CORRECTION, pas un oubli
# ══════════════════════════════════════════════════════════════════════════════════════════════
# Une première version déclarait un Receiver IS-07 sur chaque conteneur qui CONSOMME du tally
# (multiview, scope, hello_world). Modèle faux, à deux titres :
#
#   · la CARDINALITÉ — le tally d'un scope est celui de chacune de ses entrées, pas un par
#     conteneur ; un multiview à 16 tuiles a besoin des 16 ;
#   · surtout l'ENDROIT — le tally n'appartient pas au consommateur. C'est une propriété du
#     SIGNAL. Un scope n'a pas besoin qu'on lui ENVOIE un tally : il affiche celui de la source
#     qu'il regarde, et l'orchestrateur sait déjà laquelle.
#
# Les Receivers IS-07 vont donc sur les SORTIES, un par groupe BCP-002-01, au même endroit que
# les Senders que `is07.py` publie déjà. Voir TODO.md § TALLY.
